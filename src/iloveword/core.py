...
def match(data):
    ...
    main_text = purify_text(data[0][1])
    options = data[1:]
    if is_chi(main_text):
        # 将英文选项转为中文, 然后匹配……
        # Local Dict
        ...
        for i in range(len(options)):
            mres = fuzz.partial_ratio(trans_res[i], main_text)
        ...
        # 将题干转换为中文
        try:
            trans_main = sdict.query(main_text)["translation"]
        except Exception:
            return None
        print(repr(main_text), "->", repr(trans_main))
        mreses = []
        for i in range(len(options)):
